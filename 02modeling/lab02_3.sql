...
-- a. No, I would not need an additional sequence for the performer relation primary key values, although it will be helpful to have one. The same sequence can be used for relations as long as the primary key values within each realtion are unique.                                                                                 
-- b. A problem I see using sequences in a DDL command file to construct the full movies databases is a longer execution time arising from the time needed to calculate the sequence of the different values. 
...
-- Create sequence for the primary key values of the Movie relational
